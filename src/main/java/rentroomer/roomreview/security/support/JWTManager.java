...
public class JWTManager {
    ...
    private static final String CLAIM_KEY_IS_SOCIAL = "isSocial";
    ...
    public String generate(Account account) throws JWTCreationException {
        return JWT.create()
                ...
                .withClaim(CLAIM_KEY_IS_SOCIAL, account.isSocialAccount())
    ...
    public Account decode(String encodedToken) {
        ...
        try {
            decodedJWT = verifier.verify(encodedToken);
            checkExpire(decodedJWT);
            /*
            decodedJWT.getClaim(CLAIM_KEY_ID).asLong()
            decodedJWT.getClaim(CLAIM_KEY_USER_ID).asString(),
            UserRole.getRole(decodedJWT.getClaim(CLAIM_KEY_AUTHORITY).asString())
            */
            return null;
